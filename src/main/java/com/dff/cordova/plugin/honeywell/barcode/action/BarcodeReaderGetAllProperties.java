...
public class BarcodeReaderGetAllProperties extends HoneywellAction {

    private static final String TAG = "com.dff.cordova.plugin.honeywell.barcode.action.barcodeReaderGetProfileNames";
    public static final String ACTION_NAME = "barcodeReaderGetAllProperties";

    public BarcodeReaderGetAllProperties(String action, JSONArray args, CallbackContext callbackContext,
    ...
    public void run() {
        try {
            if(this.barcodeReaderManager.getInstance() != null) {
                Map<String, Object> mapAllProperties = this.barcodeReaderManager.getInstance().getAllProperties();
